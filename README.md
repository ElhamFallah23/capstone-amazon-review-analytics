# Amazon Review Analytics — Event-Driven End-to-End Data Platform
...
- Layered data modeling (raw → stage → mart)
...
### 🔐 Secure CI/CD with OIDC (No Hard-Coded Credentials)
...
5. Step Functions orchestrates Glue jobs (clean → flatten)
...
### ❄️ Snowflake External Table with Automatic Refresh
...
### 🔁 Airflow Orchestration (Extensible Design)
...
- `terraform-validate-fmt.yaml` → Validates formatting and syntax
- `terraform-pr-plan.yaml` → Runs Terraform plan on pull requests
- `terraform-post-merge-apply.yaml` → Applies changes after merge
